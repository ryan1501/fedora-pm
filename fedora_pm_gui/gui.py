...
class FedoraPmGui(QWidget):
    ...
    def install_kernel(self):
        """Install a kernel."""
        version = self.kernel_version_input.text().strip()
        if version:
            self.run_cli_command(["kernel", "install", version])
        else:
            self.run_cli_command(["kernel", "install"])

    def show_group_info(self):
        """Show information about a package group."""
        group = self.group_input.text().strip()
        if group:
            self.run_cli_command(["group", "info", group])
        else:
            QMessageBox.warning(self, "Input Required", "Please enter a group name.")
    ...
    def install_group(self):
        ...
        if group:
            reply = QMessageBox.question(
                self,
                "Confirm Installation",
                f"Install package group: {group}?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.Yes
            )
            if reply == QMessageBox.StandardButton.Yes:
                self.run_cli_command(["group", "install", group, "-y"])
        else:
            QMessageBox.warning(self, "Input Required", "Please enter a group name.")

    def show_changelog(self):
        """Show changelog for a package."""
        package = self.changelog_input.text().strip()
        if package:
            self.run_cli_command(["changelog", package])
        else:
            QMessageBox.warning(self, "Input Required", "Please enter a package name.")
